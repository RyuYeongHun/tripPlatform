	...
	<head>
		<meta charset="UTF-8">
			...
			* {
			...
			let slideIndex2 = 0; // 초기 인덱스를 0으로 설정
			...
			function showSlides1() {
			    ...
			    let initialOffset = -550; // 초기 위치를 -550px로 설정
			    ...
			    // 슬라이드 인덱스를 증가시키고 범위를 체크
			    slideIndex1++;
			    if (slideIndex1 >= slides.length) {
			        slideIndex1 = 0; // 슬라이드가 끝에 도달하면 다시 처음으로
			    }
			}

			// setInterval로 3초마다 자동 슬라이드 변경
			autoSlideInterval1 = setInterval(showSlides1, 3000);
			...
			function showSlides2() {
			    ...
			    let slideWidth2 = slides[0].offsetWidth + 60; // 슬라이드 너비와 마진을 고려
			    slideGroup.style.transition = 'transform 0.5s ease';
			    ...
			    if (slideIndex2 >= slides.length) {
			        slideIndex2 = 0; // 슬라이드가 끝에 도달하면 다시 처음으로
			    }
			}

			// setInterval로 3초마다 자동 슬라이드 변경
			autoSlideInterval2 = setInterval(showSlides2, 3000);
			...
			<c:forEach var="location" items="${nearbyLocations}">
		    	if (!(parseFloat('${location.mapY}') === mapY && parseFloat('${location.mapX}') === mapX)) {
		        	positions.push({
		            	...
		            	description: '<a href="' + window.location.origin + '/detail_view?bolgguri_id=${location.bolgguri_id}">상세페이지</a>',  
