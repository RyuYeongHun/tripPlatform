...
.question_plus button{
	margin-left:317px;
	padding: 10px 20px;
	border: 1px solid #6290fb;
	border-radius: 4px;
	font-size: medium;
	cursor: pointer;
...
$(document).ready(function() {
    ...
    // 문의하기 버튼 클릭 시 세션 확인
    $('.question_plus button').on('click', function(event) {
        ...
        <div class="content_zone_wrap">
            <div class="main">
                <div class="center">
                    ...
                    <p id="title-sub">어려운 점이나 궁금한 점이 있으신가요?</p>
                    ...
                    <div class="nav-buttons-container">
                        ...
                        <button class="nav-button" data-section="second-section">결제/영수증</button>
                        <button class="nav-button" data-section="third-section">볼거리</button>
                        
                        <form action="/plus" method="get">
                            <div class="question_plus">
                                <button class="plus">문의하기</button>
                            </div>
                        </form>
                    ...
                    <!-- 결제/영수증 섹션 -->
                    <div id="second-section" class="section-content">
                        <c:forEach var="faq" items="${faq}" varStatus="status">
                            <c:if test="${faq.category == '결제/영수증'}">
                                <div class="faq" data-index="second-${status.index}">
                                    <p class="question">[${faq.category}] ${faq.question }</p>
                                </div>
                                <div class="answer" id="answer-second-${status.index}">
                                    <pre>A.&nbsp;${faq.answer }</pre>
                                </div>
                            </c:if>
                        </c:forEach>
                    ...
                    <!-- 볼거리 섹션 -->
                    <div id="third-section" class="section-content">
                        <c:forEach var="faq" items="${faq}" varStatus="status">
                            <c:if test="${faq.category == '볼거리'}">
                                <div class="faq" data-index="third-${status.index}">
                                    <p class="question">[${faq.category}] ${faq.question }</p>
                                </div>
                                <div class="answer" id="answer-third-${status.index}">
                                    <pre>A.&nbsp;${faq.answer }</pre>
                                </div>
                            </c:if>
                        </c:forEach>
