...
<body>
	...
	<div class="view">
		<div class="view-header view-header_intro">
			...
			<div class="location">
				<ul class="list">
					...
					<li><button id="likeButton">Like</button><strong class="title">좋아요</strong> <span id="likeCount" class="text">${bolgguriDetails.likes}</span></li>
        ...
        $('#likeButton').click(function() {
            ...
            $.post(url, { bolgguri_id: bolgguri_id }, function(data) {
...
<%-- 				<caption>이용정보 홈페이지주소, 전화번호, 이용시간, 휴무일, 입장료/이용료, 화장실유무, 주차
				...
				<tbody>
					<tr id="website-row">
						...
						<td><a href="${bolgguriDetails.website}" target="blank" title="새창:아라마루전망대 홈페이지로 이동">${bolgguriDetails.website}</a></td>
					</tr>
					<tr id="tel-row">
						<th scope="row">전화번호/문의 및 안내</th>
						<td>${bolgguriDetails.tel}<br>${bolgguriDetails.infocenter}</td>
					</tr>
					<tr id="opening-hours-row">
						<th scope="row">이용시간</th>
						<td>${bolgguriDetails.opening_hours}</td>
					</tr>
					<tr id="holidays-row">
						<th scope="row">휴무일</th>
						<td>${bolgguriDetails.restdate}</td>
					</tr>
					<tr id="fee-row">
						<th scope="row">입장료/이용료</th>
						<td>${bolgguriDetails.fee != 0 ? bolgguriDetails.fee + ' (' + bolgguriDetails.chkcreditcard + ')' : '무료'}</td>
					</tr>
					<tr id="restroom-row">
						<th scope="row">화장실 유무</th>
						<td>${bolgguriDetails.restroom_yn == 'N' ? '없음' : '있음'}</td>
					</tr>
					<tr id="parking-row">
						<th scope="row">주차 안내</th>
						<td><c:choose>
							<c:when test="${bolgguriDetails.parking_cnt == 0}">
								없음
							</c:when>
							<c:otherwise>
					...
					<tr id="pet-allow-row">
						...
						<td><c:choose>
							...
							<c:otherwise>
								${bolgguriDetails.pet_allow}
							</c:otherwise>
						</c:choose></td>
					</tr>
					<tr id="expagerange-row">
						<th scope="row">사용가능 연령</th>
						<td>${bolgguriDetails.expagerange}</td>
					</tr>
					<tr id="expguide-row">
						<th scope="row">체험안내</th>
						<td>${bolgguriDetails.expguide}</td>
					</tr>
					<tr id="special-note-row">
						<th scope="row">특이사항</th>
						<td>${bolgguriDetails.special_note}</td>
						</tr>
				</tbody>
			</table>
		</div>
		<script>
		// 이용정보 script
		// 각 tr을 가져와서 td 값이 null, undefined, 빈 문자열인 경우 숨김 처리
			const rows = [
				{ id: "website-row", value: "${bolgguriDetails.website}" },
				{ id: "tel-row", value: "${bolgguriDetails.tel}" },
				{ id: "opening-hours-row", value: "${bolgguriDetails.opening_hours}" },
				{ id: "holidays-row", value: "${bolgguriDetails.restdate}" },
				{ id: "fee-row", value: "${bolgguriDetails.fee}" },
				{ id: "restroom-row", value: "${bolgguriDetails.restroom_yn}" },
				{ id: "parking-row", value: "${bolgguriDetails.parking_cnt}" },
				{ id: "pet-allow-row", value: "${bolgguriDetails.pet_allow}" },
				{ id: "expagerange-row", value: "${bolgguriDetails.expagerange}" },
				{ id: "expguide-row", value: "${bolgguriDetails.expguide}" },
				{ id: "special-note-row", value: "${bolgguriDetails.special_note}" }
			];
