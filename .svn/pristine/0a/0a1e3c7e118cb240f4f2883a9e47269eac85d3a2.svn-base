...
public class YhController {
	 ...
	 */
     ...
     public String home(HttpSession session, HttpServletRequest request, Model model) {
         ...
         //mw 내용입니다.
         List<MwBolgguriRegionDto> mwBolgguriRegionDtos = mwService.regionSelectAll(); // 지역 선택 버튼 리스트
         List<MwBolgguriDto> mwBolgguriDtos = mwService.selectAll(null, null, "2", 0, 10, null);
         List<String> category = new ArrayList<>();
         category.add("A03");
         List<MwBolgguriDto> mwBolgguriDtos2 = mwService.selectAll(null, category, "2", 0, 10, null);
	...
	public int unlikePost(@RequestParam("bolgguri_id") Integer bolgguri_id, HttpSession session) throws Exception {
	    ...
	    @ResponseBody
	    public ResponseEntity<Map<String, Object>> deleteReview(@RequestParam("review_id") Integer review_id, HttpSession session) throws Exception {
	        String user_id = (String) session.getAttribute("user_id");
	        yhService.reviewDelete(review_id, user_id);
